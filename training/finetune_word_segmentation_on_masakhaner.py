...
from helpers import get_model_hyperparams, SequenceLabelingDataCollator, \
    ShibaWordSegArgs, get_base_shiba_state_dict, load_and_fix_state_dict
import torchmetrics
...
def main():
    ...
    masakhaner_dataset = masakhaner_dataset.map(process_example, remove_columns=list(masakhaner_dataset['train'][0].keys()))
    ...
    trainer = Trainer(model=model,
                      ...
                      train_dataset=masakhaner_dataset['train'],
                      eval_dataset=masakhaner_dataset['validation'],
                      compute_metrics=compute_metrics
                      )

    trainer.train()
    posttrain_metrics = trainer.predict(masakhaner_dataset['test']).metrics
